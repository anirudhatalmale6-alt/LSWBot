...
module.exports = function (parent, chanName) {
	...
	cmdHandler.ayuda = function (args, data) {
	...
	cmdHandler.sendmoney = function (args, data) {	
		...
		if (isNaN(cantidad) || cantidad < 1) {
			var message = "La cantidad debe ser un numero positivo (sin el $ o el .00)";
		...
		if (destiny == undefined) {
			var message = "Debes especificar el destinatario. Ejemplo: !sendmoney 3 to Kenia Nya";
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
			return 0;
		}
		client.hgetall(data.character, function (err, chara1) {
			if (chara1 == null) {
				var message = "No estas registrado(a).";
				data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
				return 0;
			}
			client.hgetall(destiny, function (err, chara2) {
				if (chara2 == null) {
					var message = destiny + " no fue encontrado(a).";
					data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
					return 0;
				}
				if (parseInt(chara1.Gold) >= cantidad) {
					...
					var message = data.character + " ha enviado $" + cantidad + ".00 a " + destiny + ".";
				} else {
		...
		if (Combate.started == false || data.publico == false) {
			client.hgetall(data.character, function (err, chara) {
				if (chara != null) {
					...
					var message = "No estas registrado(a), usa !register primero.";
				}
				data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
			});
		} else {
			fChatLibInstance.sendPrivMessage(data.character, "Hay un combate en progreso, este comando ha sido deshabilitado en publico para prevenir spam, usalo en privado.");
		}
	}
	...
	cmdHandler.cardcode = function (args, data) {
		client.hgetall(data.character, function (err, chara) {
			if (chara != null) {
				...
				fChatLibInstance.sendMessage("No estas registrado(a), usa !register primero.", channel);
			}
		});
	}
	...
	cmdHandler.sheet = function (args, data) {
		client.hgetall(data.character, function (err, chara) {
			if (chara != null) {
				...
				fChatLibInstance.sendPrivMessage(data.character, "No estas registrado(a), usa !register primero.");
			}
		});
	}
	...
	cmdHandler.buy = function (args, data) {
		...
		if (itemAsked == -1) {
			var message = "El objeto '" + args + "' no fue encontrado.";
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
			return 0;
		}
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.Gold >= costo) {
				...
				var message = "El objeto " + args + " ha sido añadido a tu lista. Tienes $" + pj.Gold + ".00 restante.";
			} else {
				var message = "No puedes costear eso, tu tienes $" + pj.Gold + ".00";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.sell = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.removeItem(itemAsked)) {
				...
				pj.addGold(ganancia);
				client.hmset(data.character, pj.getSaveFile());
				var message = "El objeto " + args + " ha sido vendido! " + data.character + " obtuvo $" + ganancia + ".00. (Los objetos se recompran a mitad de precio debido a costos de sanitizacion.)";
			} else {
				var message = "El objeto " + args + " no fue encontrado o es un objeto inicial.";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.equip = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.equip(itemAsked)) {
				client.hmset(data.character, pj.getSaveFile());
				var message = "El objeto " + args + " ha sido equipado.";
			} else {
				var message = "El objeto " + args + " no fue encontrado.";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
	...
	cmdHandler.register = function (args, data) {
		client.hexists(data.character, "name", function (err, reply) {
			if (reply == 0) {
				...
				var message = data.character + " se ha registrado!";
			} else {
				var message = "Ya estabas registrado(a)";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.resetcharacter = function (args, data) {
		client.hgetall(data.character, function (err, chara) {
			if (chara != null) {
				...
				var message = data.character + " hizo un reset!";
			} else {
				var message = "Registrate primero.";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.train = function (args, data) {
		if (data.publico == true) {
			fChatLibInstance.sendPrivMessage(data.character, "Este comando solo funciona en privado, usalo aqui.");
			return 0;
		}
		...
		if (!( ( (part == "lips" || part == "fingers" || part == "tits" || part == "sex" || part == "ass" || part == "feet") && (atkORdef == "attack") ) || ( (part == "lips" || part == "tits" || part == "sex" || part == "ass") && (atkORdef == "defense") ) )) {
			fChatLibInstance.sendPrivMessage(data.character, "Alguna palabra esta mal escrita. Usa una de las siguientes: lips, fingers, tits, sex, ass, feet. Ejemplo: !train attack on fingers");
			return 0;
		}
		...
		client.hgetall(data.character, function (err, chara) {
			...
			var pj = new Personaje(chara);
			if (pj.train(destiny)) {
				fChatLibInstance.sendPrivMessage(data.character, "[eicon]training[/eicon] Entrenamiento exitoso! Tu ahora tienes " + pj[destiny] + " puntos en " + atkORdef + " en " + part + ".");
				client.hmset(data.character, pj.getSaveFile());
			} else {
				fChatLibInstance.sendPrivMessage(data.character, "Dinero insuficiente (Costo: $10.00) o esa parte esta al maximo. (Maximo sin items: 5).");
			}
		});
	}
	...
	cmdHandler.indulge = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.indulge(destiny)) {
				fChatLibInstance.sendPrivMessage(data.character, "Disminucion exitosa! Tu ahora tienes " + pj[destiny] + " puntos en " + atkORdef + " en " + part + ".");
				client.hmset(data.character, pj.getSaveFile());
			} else {
				fChatLibInstance.sendPrivMessage(data.character, "No puedes reducir mas ese stat. (Minimo sin items: -3).");
			}
		});
	}
	...
	cmdHandler.ready = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			var pj = new Personaje(chara);
			if (Combate.started == true) { fChatLibInstance.sendMessage("Ya hay un combate en progreso.", channel); return 0; }
			if (Combate.addActor(pj)) {
				hoja = "\n";
				hoja += "[color=gray]═════════════ ⭐ [color=purple]Ring de lucha sexual[/color] ⭐ ═════════════\n";
				hoja += "          [icon]" + data.character + "[/icon] Ha entrado al ring![/color]\n";
				fChatLibInstance.sendMessage(hoja, channel);
				if (Combate.actores.length == 2) {
					Combate.initiative();
					var message = "\n[icon]Bot Announcer[/icon][color=gray] Bienvenidos y bienvenidas al ring de luchas sexuales! Hoy tenemos a " + Combate.actores[0].name + " vs " + Combate.actores[1].name + "! Que comienze la lucha![/color]";
					message += Combate.status();
					fChatLibInstance.sendMessage(message, channel);
				}
			} else {
				fChatLibInstance.sendMessage("Ya estas dentro.", channel);
			}
		});
	}
	...
	cmdHandler.attack = function (args, data) {
		if (data.publico) {
			if (Combate.started == false) { fChatLibInstance.sendMessage("El combate no ha comenzado aun.", channel); return 0; }
			if (Combate.activeActor() != data.character) { fChatLibInstance.sendMessage("No es tu turno.", channel); return 0; }
			var arr = args.split(' to ');
			var origin = arr[0];
			var destiny = arr[1];
	...
	cmdHandler.pass = function (args, data) {
		if (data.publico) {
			...
			if (Combate.activeActor() != data.character) { fChatLibInstance.sendMessage("No es tu turno.", channel); return 0; }
			Combate.nextActor();
			var message = "\n[icon]Bot Announcer[/icon][color=gray]" + Combate.actores[1].name + " ha pasado su turno![/color]";
			message += Combate.status();
			fChatLibInstance.sendMessage(message, channel);
		}
	}
	...
	cmdHandler.endfight = function (args, data) {
		if (data.publico) {
			if (Combate.actores.length > 0) {
				Combate.reset();
				fChatLibInstance.sendMessage("Fin del combate!", channel);
			}
		} else {
			if (gyms[data.character] != undefined) {
				gyms[data.character] = undefined;
				fChatLibInstance.sendPrivMessage(data.character, "Fin del entrenamiento!");
			}
		}
	}
...
function generar_ayuda() {
	var hoja = fs.readFileSync("./help2.txt", "utf8");
	return hoja;
}
...
function generar_hoja_chica(pj) { 
	...
	var hoja = "\n[color=gray]";
	hoja +="══════════════════════════════════════════════════\n";
	hoja +="                                 ⭐ ⭐ [color="+color1+"][b]Ficha publica[/b][/color] ⭐ ⭐\n";
	hoja +="══════════════════════════════════════════════════\n[color="+color2+"]"; //
	hoja +="                     [icon]" + pj.name + "[/icon]\n";
	hoja +="          [b]Nombre:[/b] " + pj.name + " | [b]Dinero:[/b] $" + pj.Gold + ".00\n";
	hoja +="          [b]Ataque:[/b] " + pj.atklips + " Lips, " + pj.atkfingers + " Fingers, " + pj.atktits + " Tits, " + pj.atksex + " Sex, " + pj.atkass + " Ass, " + pj.atkfeet + " Feet.\n";
	hoja +="          [b]Defensa:[/b] " + pj.deflips + " Lips, " + pj.deftits + " Tits, " + pj.defsex + " Sex, " + pj.defass + " Ass.\n";
	hoja +="          [b]Sextoy:[/b] " + pj.weapon.name + ". " + generar_stats(pj.weapon) + "\n";
	hoja +="          [b]Atuendo:[/b] " + pj.armor.name + ". " + generar_stats(pj.armor) + "\n";
	hoja +="          [b]Accesorio:[/b] " + pj.item.name + ". " + generar_stats(pj.item) + "\n";
	hoja +="          [b]Extra:[/b] " + pj.flavorr.name + ". " + generar_stats(pj.flavorr) + "\n";
	hoja +="          [b]Adiccion:[/b] " + pj.addiction.name + ". " + generar_stats(pj.addiction) + "\n";
	hoja +="[/color][/color]";
	return hoja;
}
...
function generar_hoja(pj) {
	...
	var hoja = "\n[color=gray]";
	hoja +="══════════════════════════════════════════════════\n";
	hoja +="                                 ⭐ ⭐ [color="+color1+"][b]Detalles generales[/b][/color] ⭐ ⭐\n";
	hoja +="══════════════════════════════════════════════════\n[color="+color2+"]"; //
	hoja +="                     [icon]" + pj.name + "[/icon]\n";
	hoja +="          [b]Nombre:[/b] " + pj.name + " | [b]Dinero:[/b] $" + pj.Gold + ".00\n[/color]";
	hoja +="══════════════════════════════════════════════════\n";
	hoja +="                                 ⭐ ⭐ [color="+color1+"][b]Ataque / Defensa[/b][/color] ⭐ ⭐\n";
	hoja +="══════════════════════════════════════════════════\n[color="+color2+"]"; //
	...
	hoja +="══════════════════════════════════════════════════\n";
	hoja +="                                 ⭐ ⭐ [color="+color1+"][b]Equipamiento[/b][/color] ⭐ ⭐\n";
	hoja +="══════════════════════════════════════════════════\n[color="+color2+"]"; //
	hoja +="          [b]Sextoy:[/b] " + pj.weapon.name + "\n";
	hoja +="                    " + Weapon_stats + "\n";
	hoja +="          [b]Atuendo:[/b] " + pj.armor.name + "\n";
	hoja +="                    " + Armor_stats + "\n";
	hoja +="          [b]Accesorio:[/b] " + pj.item.name + "\n";
	hoja +="                    " + Item_stats + "\n";
	hoja +="          [b]Extra:[/b] " + pj.flavorr.name + "\n";
	hoja +="                    " + Flavor_stats + "\n";
	hoja +="          [b]Adiccion:[/b] " + pj.addiction.name + "\n";
	hoja +="                    " + Addiction_stats + "\n[/color]";
	hoja +="══════════════════════════════════════════════════\n";
	hoja +="                                 ⭐ ⭐ [color="+color1+"][b]Inventario[/b][/color] ⭐ ⭐\n";
	hoja +="══════════════════════════════════════════════════\n[color="+color2+"]"; //
