...
module.exports = function (parent, chanName) {
	...
	cmdHandler.buystatpoint = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.Gold >= 10) {
				...
				message = "You bought a stat point! You now have $" + pj.Gold + ".00 left and " + pj.sp + " stat points.";
			}
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.train = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.train(destiny)) {
				...
				fChatLibInstance.sendPrivMessage(data.character, "Not enough stat points (You need: "+costo+") or you have maxed out that part (Max wihout items: 8).");
			}
		});
	}
	...
	//Listeners
	//****************************
	if (channel == "ADH-c152868032e8d14d5bbc") {
...
function generar_stats(objeto) {  
	...
	if (objeto.deffeet != null)		{ message.push(signo(objeto.deffeet) + " def on ass"); }
