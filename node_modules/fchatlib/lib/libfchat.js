...
class FChatLib {
    ...
    constructor(options){
        ...
        this.addPrivateMessageListener(this.privateCommandListener);
    ...
    joinChannelOnConnect() {
        Object.keys(parent.channels).forEach(function(room){
			if (parent.channels[room].active == "false") { return 0; }
			if (parent.channels[room].title && parent.channels[room].title.substring(0, 8) == "Private ") {
    ...
    privateCommandListener(parent, args) {
        var mainChannel = parent.config.room;
        var handler = parent.commandHandlers[mainChannel];
        if (!handler) {
            var keys = Object.keys(parent.commandHandlers);
            if (keys.length > 0) { handler = parent.commandHandlers[keys[0]]; }
        }
        if (handler) {
            try {
                handler(parent, args, mainChannel);
            }
            catch(ex){
                console.log(ex);
            }
	...
	connect() {
        ...
        request.post({ url: 'https://www.f-list.net/json/getApiTicket.php', form: { account: parent.config.username, password: parent.config.password } }, function (err, httpResponse, body) {
			...
			if (response.error && response.error.length > 0) {
				console.log("getApiTicket error: " + response.error);
				setTimeout(parent.connect, 15000);
				return 0;
			}
			...
			console.log("IDN account: " + parent.config.username + ", character: " + parent.config.character + ", ticket length: " + (ticket ? ticket.length : "MISSING"));
	...
	*/
    ...
    sendWS(command, object) {
        if (parent.ws.readyState === WebSocket.OPEN) {
			if (command === 'IDN') { var payload = command + ' ' + JSON.stringify(object); console.log("IDN payload length: " + payload.length); }
    ...
    softRestart(){
		...
		parent.addPrivateMessageListener(parent.privateCommandListener);
    }
