...
var Personaje = require('./etc/personaje4.js');
...
module.exports = function (parent, chanName) {
	...
	cmdHandler.meow = function (args, data) {
		...
		fChatLibInstance.sendMessage("Meow meow~", channel);
	}
	...
	cmdHandler.outfits = function (args, data) {
		if (Combate.started == false || data.publico == false) {
			let items = requireNew('./etc/shop3.js');
			let message = generar_tienda(items, 0, 499, "Outfit store", "👘");
...
function generar_ayuda() {
	return "please check my profile to see the full updated help~";
