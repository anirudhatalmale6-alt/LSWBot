...
var Personaje = require('./etc/personaje2.js');
...
module.exports = function (parent, chanName) {
	...
	cmdHandler.sellstatpoint = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			let message = "You can't sell stat points.";
			data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
		});
	}
	...
	cmdHandler.train = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			let pj = new Personaje(chara);
			if (pj.train(destiny)) {
				fChatLibInstance.sendPrivMessage(data.character, "Training successful! You now have " + pj[destiny] + " points in " + atkORdef + " on " + part + ", and "+pj.sp+" stat points left.");
				client.hmset(data.character, pj.getSaveFile());
			} else {
				let costos = [0,1,1,2,2,3,4,5,0]; let stat = pj["b"+destiny]; let costo = costos[stat];
				fChatLibInstance.sendPrivMessage(data.character, "Not enough stat points (You need: "+costo+") or you have maxed out that part (Max wihout items: 7).");
			}
		});
	}
	...
	cmdHandler.indulge = function (args, data) {
		...
		client.hgetall(data.character, function (err, chara) {
			...
			if (pj.indulge(destiny)) {
				fChatLibInstance.sendPrivMessage(data.character, "Indulging successful! You now have " + pj[destiny] + " points in " + atkORdef + " on " + part + ", and "+pj.sp+" stat points left.");
				client.hmset(data.character, pj.getSaveFile());
			} else {
				fChatLibInstance.sendPrivMessage(data.character, "You have the least ammount on that part (Min wihout items: 0).");
			}
		});
	}
	...
	cmdHandler.dance = function (args, data) { cmdHandler.attack("body to sight", data); }
	...
	cmdHandler.attack = function (args, data) {
		if (data.publico) {
			if (Combate.started == false) {
				//fChatLibInstance.sendMessage("Combat hasn't started yet.", channel); return 0; *********************************************************************
				let arr = args.split(' to ');
				let origin = arr[0];
				let destiny = arr[1];
...
function generar_hoja_chica(pj) { 
	...
	hoja +="          [b]Name:[/b] " + pj.name + " | [b]Money:[/b] $" + pj.Gold + ".00 | [b]Stat points:[/b] " + pj.sp + " | [b]Wins/Losses:[/b] " + pj.wins + "/" + pj.loses + "\n";
