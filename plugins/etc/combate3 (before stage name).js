...
var crit = false;
var icon = "[icon]Milly The Succubot[/icon]";

function Combate() {
	this.mode = ["please","building them up by","lust","pink","cum","sexfight"];
	this.modeflag = true;
	this.betting = false;
	this.bettingPeople = [];
	this.turnCount = 1;
	...
	this.changeMode = function() {
		if (this.modeflag) {
			this.mode = ["hurt","dealing","hit","red","KO","combat"]; this.modeflag = false;
			return "Rough combat mode enabled!";
		} else {
			this.mode = ["please","building them up by","lust","pink","cum","sexfight"]; this.modeflag = true;
	...
	this.addActor = function(actor, hp, team) {
		...
		if (team == "red" || team == "blue") {
			...
			hoja += "          [icon]" + actor.name + "[/icon][eicon]"+rank+"-rank[/eicon] Has entered the ring on team [color="+team+"]"+team+"![/color][/color]";
			...
			hoja += "          [icon]" + actor.name + "[/icon][eicon]"+rank+"-rank[/eicon] Has entered the ring![/color]";
		}
		var cantidad = parseInt(hp);
		if (!isNaN(cantidad) && cantidad > 0 && cantidad < 201) {
	...
	this.attack = function(origin1, destiny1, letra, name, statusSkip, crowd) {
		let isSextoy = false;
		if (origin1 == "sextoy") {
			isSextoy = true;
			if (this.actores[0].weapon.atklips !== undefined) { origin1 = "lips"; }
			if (this.actores[0].weapon.atkfingers !== undefined) { origin1 = "fingers"; }
			if (this.actores[0].weapon.atktits !== undefined) { origin1 = "tits"; }
			if (this.actores[0].weapon.atksex !== undefined) { origin1 = "sex"; }
			if (this.actores[0].weapon.atkass !== undefined) { origin1 = "ass"; }
			if (this.actores[0].weapon.atkfeet !== undefined) { origin1 = "feet"; }
		}
		...
		if (origin != "" && destiny != "") {
			...
			if (this.teamsf == true) {
				...
				if (letra == "me") { var defensor = this.actores[0]; } else { var defensor = this.actores[1]; var skipturn = true; }
				if (crowd === undefined) { var atacante = this.actores[0]; var skipturn = false; } else {
					var atacante = crowd; var skipturn = true;
			...
			var message = "\n"+icon+"[color=pink][b]" + atacante.stageName + "[/b][/color] used their [color=pink][b]" + origin1 + "[/b][/color] to "+this.mode[0]+" [color=pink][b]" + defensor.stageName + "'s " + destiny1 + "[/b][/color], "+this.mode[1]+" [color="+this.mode[3]+"][b]" + damage + " "+this.mode[2]+" points![/b][/color] "+comment(damage);
	...
	this.strip = function(letra, name) {
		...
		if (dado <= probabilidad) {
			defensor.stripMe();
			var message = "\n"+icon+"[color=pink][b]"+atacante.stageName+"[/b][/color] managed to strip [color=pink][b]"+defensor.stageName+"'s[/b][/color] outfit! Things are getting hot now!";
			if (this.sMatch) {
