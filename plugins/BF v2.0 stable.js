...
/////////////////////////////////////////////////////////////////////////////////////		Cambiar indulge para que solo funcione con defensas
...
module.exports = function (parent, chanName) {
	...
	cmdHandler.givemoney = function (args, data) { //
		if (fChatLibInstance.isUserChatOP(data.character, channel)) {
			...
			client.hgetall(destiny, function (err, chara) {
				if (chara != null) {
					chara.Gold = cantidad + parseInt(chara.Gold);
					client.hmset(destiny, chara);
					fChatLibInstance.sendMessage("Added $" + cantidad + ".00 to " + destiny, channel);
				} else {
					fChatLibInstance.sendMessage("Character " + destiny + " wasn't found.", channel);
				}
			});
		} else {
			fChatLibInstance.sendMessage("You are not an admin.", channel);
		}
	}
	
	cmdHandler.giveitem = function (args, data) { //
		if (fChatLibInstance.isUserChatOP(data.character, channel)) {
			...
			var itemAsked = busca(items, objeto);
			if (itemAsked != -1) {
				client.hgetall(destiny, function (err, chara) {
					if (chara != null) {					
						var pj = new Personaje(chara);
						pj.giveItem(itemAsked);
						client.hmset(destiny, pj.getSaveFile());
						fChatLibInstance.sendMessage("The " + objeto + " has been given to " + destiny + ".", channel);
					} else {
						fChatLibInstance.sendMessage("Character " + destiny + " wasn't found.", channel);
					}
				});
			} else {
				fChatLibInstance.sendMessage("The item '" + objeto + "' wasn't found.", channel);
			}
		} else {
			fChatLibInstance.sendMessage("You are not an admin.", channel);
		}
	...
	cmdHandler.removeitem = function (args, data) { //
		if (fChatLibInstance.isUserChatOP(data.character, channel)) {
			
			var arr = args.split(' from ');
			var objeto = arr[0];
			var destiny = arr[1];
				...
				client.hgetall(destiny, function (err, chara) {
					if (chara != null) {
						...
						if (pj.removeItem(itemAsked)) {
							client.hmset(destiny, pj.getSaveFile());
							fChatLibInstance.sendMessage("The " + objeto + " has been taken away from " + destiny + ".", channel);
						} else {
							fChatLibInstance.sendMessage(destiny + " didn't have a " + objeto + " or is a starter item.", channel);
						}
					} else {
						fChatLibInstance.sendMessage("Character " + destiny + " wasn't found.", channel);
					}
		...
		if (fChatLibInstance.isUserChatOP(data.character, channel)) {
			client.hgetall(args, function (err, chara) {
				if (chara != null) {
					...
					fChatLibInstance.sendPrivMessage(data.character, args + " wasn't found.");
				}
			});
		} else {
			fChatLibInstance.sendMessage("You are not an admin.", channel);
	...
	cmdHandler.viewbotstatus = function (args, data) {
		if (fChatLibInstance.isUserChatOP(data.character, channel)) {
			...
			message += "Combat: " + Combate.started;
			fChatLibInstance.sendPrivMessage(data.character, message);
		}
	}
	...
	cmdHandler.card = function (args, data) {
		if (Combate.started == false || data.publico == false) {
			...
			fChatLibInstance.sendPrivMessage(data.character, "There's a combat in progress, command disabled to prevent spam.");
		}
	}
		...
		if (itemAsked != -1) {
			client.hgetall(data.character, function (err, chara) {
				if (chara != null) {
					...
					var message = "You're not listed, leave and rejoin the main room to get added.";
				}
				data.publico ? fChatLibInstance.sendMessage(message, channel) : fChatLibInstance.sendPrivMessage(data.character, message);
			});
		} else {
	...
	cmdHandler.register = function (args, data) {
		client.hexists(data.character, "name", function (err, reply) {
			if (reply == 0) {
				...
				nuevo.ownedItems = "0,100,200,300";
	...
	cmdHandler.train = function (args, data) {
		if (Combate.started == false || data.publico == false) {
			...
			if ( ( (part == "lips" || part == "fingers" || part == "tits" || part == "sex" || part == "ass" || part == "feet") && (atkORdef == "attack") ) || ( (part == "lips" || part == "tits" || part == "sex" || part == "ass") && (atkORdef == "defense") ) ) {
				...
				fChatLibInstance.sendPrivMessage(data.character, "Wrong spelling of body parts or attack/defense. Use one of the following: lips, fingers, tits, sex, ass, feet.");
			}
		} else {
			fChatLibInstance.sendPrivMessage(data.character, "This command has moved to private only, use it here.");
		}
	}
			...
			client.hgetall(data.character, function (err, chara) {
				if (chara != null) {
					...
					if (Combate.started == false) {
						...
						fChatLibInstance.sendMessage("Combat already started.", channel);
					}
				} else {
					fChatLibInstance.sendMessage("You're not registered, use !register to join the club.", channel);
				}
				...
				if (chara != null) {
					...
					if (Combate.started == false) {
						if (Combate.addActor(pj)) {
							hoja = "\n";
							hoja += "[color=gray]═════════════ ⭐ [color=purple]Sexfight ring[/color] ⭐ ═════════════\n";
							hoja += "          [icon]" + data.character + "[/icon] Has entered the ring! (50 lust points only)[/color]\n";
							fChatLibInstance.sendMessage(hoja, channel);
							if (Combate.actores.length == 2) {
								Combate.initiative();
								var message = "\n[icon]Bot Announcer[/icon][color=gray] Welcome to the sexfight ring! Today we have " + Combate.actores[0].name + " vs " + Combate.actores[1].name + "! Let the match begin![/color]";
	...
	cmdHandler.attack = function (args, data) {
		if (data.publico) {
			if (Combate.started) {
				...
				var origin = arr[0];
				var destiny = arr[1];
				if (Combate.activeActor() == data.character) {
				...
				fChatLibInstance.sendMessage("Combat hasn't started yet.", channel);
			}
		} else {
			if (gyms[data.character] != null) {
				...
				var origin = arr[0];
				var destiny = arr[1];
				///////////////////////////////////////////////////////////ataque del player hacia el bot
				var message = gyms[data.character].attack(origin, destiny);
				///////////////////////////////////////////////////////////
				if (message != "Invalid body parts. Example of valid ones: lips, fingers, tits, sex, ass, feet. Also remember to use 'to', example: !attack tits to lips. To see the full list, check the bot profile.") {
					...
					if (gyms[data.character].started) {
						//el bot sigue vivo
						message += resultado_logica.message1 + resultado_logica.message2;
						///////////////////////////////////////////////////////////ataque del bot hacia el player
						message += gyms[data.character].attack(resultado_logica.origin2, resultado_logica.destiny2);
						///////////////////////////////////////////////////////////
						if (gyms[data.character].started == false) {
							//el player ha muerto XD
							message += resultado_logica.win;
							client.hgetall(gyms[data.character].actores[0].name, function (err, chara2) {
								var pj2 = new Personaje(chara2);
								pj2.addGold(1);
								client.hmset(pj2.name, pj2.getSaveFile());
							});
						...
						client.hgetall(gyms[data.character].actores[1].name, function (err, chara1) {
							var pj1 = new Personaje(chara1);
							pj1.addGold(2);
							client.hmset(pj1.name, pj1.getSaveFile());
						});
	...
	cmdHandler.saveFight = function (args, data) {
		if (data.publico) {
			...
			if (Combate.started) {
				if (Combate.actores[0].name == persona || Combate.actores[1].name == persona || persona == "esta persona es un administrador con poderes sobre humanos") {
					...
					saveData(data);
					Combate.reset();
					var message = "Match saved! To get a list of saved matches, use !savedfights";
				} else {
					var message = "You're not a participant of the current match or an admin.";
				}
			} else {
				var message = "There is no match going on to be saved.";
			}
			fChatLibInstance.sendMessage(message, channel);
		}
	}
		...
		if (data.publico) {
			...
			if (data.length > 0) {
				for (var i = 0; i < data.length; i++) {
					message += "          ► " + (i+1) + ": " + data[i].nombre + "\n";
				}
			} else {
				message += "          ► No saved fights found.\n";
			}
	...
	cmdHandler.loadFight = function (args, data) {
		if (data.publico) {
			...
			if (fChatLibInstance.isUserChatOP(data.character, channel)) { persona = "esta persona es un administrador con poderes sobre humanos"; }
			if (Combate.started) {
				var message = "There is a match already going on.";
			}
			if (!isNaN(numero) && numero > 0) {
				var data = loadData();
				if (data.length == 0) {
					var message = "There are no saved fights.";
				}
				if (numero > data.length) {
					var message = "You selected a number higher than the maximum number of saved fights";
				}
				if (data.length != 0 || numero <= data.length || data[(numero - 1)].actores[0] == persona || data[(numero - 1)].actores[1] == persona || persona == "esta persona es un administrador con poderes sobre humanos") {
					var message = "Resuming match!\n";
					client.hgetall(data[(numero - 1)].actores[0], function (err, chara1) {
						client.hgetall(data[(numero - 1)].actores[1], function (err, chara2) {
							var pj1 = new Personaje(chara1);
							var pj2 = new Personaje(chara2);
							pj1.removeHP(pj1.HP - data[(numero - 1)].HP[0]);
							pj2.removeHP(pj2.HP - data[(numero - 1)].HP[1]);
							Combate.addActor(pj1);
							Combate.addActor(pj2);
							data.splice((numero - 1), 1);
							saveData(data);
							Combate.startFight();
							message += Combate.status();
							fChatLibInstance.sendMessage(message, channel);
					...
					fChatLibInstance.sendMessage("You're not a participant of that match or an admin.", channel);
				}
			} else {
				fChatLibInstance.sendMessage("Save slot number invalid.", channel);
			}
		}
	}
	...
	cmdHandler.succuboss = function (args, data) {
		if (args == "male" || args == "female" || args == "shemale" || args == "herm") {
						...
						if (chara != null) {
							if (gyms[data.character] === undefined) {
								...
								if (pj.dailyCheck()) {
									...
									var entrena = new combate();
									gyms[data.character] = entrena;
									...
									fChatLibInstance.sendPrivMessage(data.character, "You reached the maximum ammount of times you can fight the bot per day, come back tomorrow~");
								}
							} else {
								fChatLibInstance.sendPrivMessage(data.character, "Training already started! Use !status to see where you are or !endfight to stop the training match.");
							}
						} else {
							//bot not found?
						}
			...
			fChatLibInstance.sendPrivMessage(data.character, "You need to specify your gender. Options: male, female, shemale and herm.");
		}
	}
	...
	cmdHandler.trainingdummy2 = function (args, data) {
		if (args == "male" || args == "female" || args == "shemale" || args == "herm") {
			client.hgetall(data.character, function (err, chara) {
				if (chara != null) {
					client.hgetall("Sissy Robert", function (err, dummy) {
						if (chara != null) {
							if (gyms[data.character] === undefined) {
								var pj = new Personaje(chara);
								if (pj.dailyCheck()) {
...
function logica_dummy(data, origin, destiny, botname) {
	...
	if (botname == "milly") {
		...
		//1, calcular el emote de recepción de daño
		var message1 = "[i]Sissy Robert moans as you tease him![/i]\n";
		//2, escojer un ataque y destino al azar
		var origenes = ["lips","fingers","tits","sex","ass","feet"];
		var destinos = ["lips","tits","sex","ass"];
		var origin2 = origenes[Math.floor(Math.random() * 6)];
		var destiny2 = destinos[Math.floor(Math.random() * 4)];
		//3, imprimir el emote apropiado
		var message2 = "[i]But then, Sissy Robert sends a sex attack at you![/i]";
		var message3 = "\n[i]Robert laughs to himself. 'You want more? Okay... come here, Cutie.' He beckons with one hand, staring hungrily at your body and licking his lips.[/i]\n";
	}
	var resultado_logica = { message1: message1, message2: message2, message3: message3, origin2: origin2, destiny2: destiny2, win: win, loss: loss };
...
function entrada(lista, indice, minId, maxId) {
	...
	if (objeto.id >= minId && objeto.id <= maxId) {
		//genera entrada
		message += "          ► " + objeto.name + "\n"
		message += "                    $ " + objeto.Gold + ".00 " + generar_stats(objeto) + "\n";
	}
	return message;
}
